module "cosmosdb" {
  ...
  name                = "${var.name_prefix}-${random_string.resource_suffix.result}"
  location            = azurerm_resource_group.foggykitchen_rg.location
  resource_group_name = azurerm_resource_group.foggykitchen_rg.name

  public_network_access_enabled = false
  key_vault_key_id              = module.cosmosdb_cmk_key.versionless_id
  default_identity_type         = "UserAssignedIdentity=${module.cosmosdb_cmk_identity.id}"

  identity = {
    type = "UserAssigned"
    identity_ids = [
      module.cosmosdb_cmk_identity.id
    ]
  }
  ...
  depends_on = [
    module.private_dns,
    module.cosmosdb_cmk_key,
    module.cosmosdb_cmk_rbac
  ]
}
